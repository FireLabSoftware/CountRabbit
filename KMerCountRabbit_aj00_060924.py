                ...
                for i1,L1 in enumerate(myIter):
                    if i1%DataCycle1==1:
                        ...
                        if i1>1 and i1%ReportLineGranularity1==1:
                            vLog('Counting Progress ' +myfn1+'Sample:'+sn+' RawReads:'+str(i1//DataCycle1)+' Matched:'+str(matchCount))
                        TotalReads += 1
                        if Paired:
                            L2 = L1[1].strip()
                            L1 = L1[0].strip()
...
AllFields1 = [HeatMapColorField0,]+ExtraTDVFields0+HeatMapTextFields0
...
## Start of "duplicated block"
        ...
        for (n11,f11) in D11:
            ...
            if not(Express1):
                ...
                if not(FoundPublicK1) or PrivateOnly1: continue
                if FirstDibs1:
...
x11 = 0
vclearance1 = int(round(vscale1))
vrect(x1=x11+4*hscale1,x2=x11+8*hscale1,y1=y11-2*vscale2,y2=y11-6*vscale2,fill='lightyellow',stroke='black',strokewidth=2.0)
for p1,j1 in enumerate(HeatMapTextFields0):
    fontsize3 = str(2*vclearance1)
    vtext(text=HeatMapTextFields1[p1],xc=x11+6*hscale1,y1=y22-3*vclearance1,color=black,font='dejavusansbold '+fontsize3)
    y22 -= 4*vclearance1
    vclearance1 = int(round(vclearance1/1.33))
y11 = 0
x11 = vscale1//2
