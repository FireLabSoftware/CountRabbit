...
## ->KCR Takes
## -   a reference (fasta) file and
## -   a set of data (fasta or fastq) files with multiple contigs or segments
## -       and returns a table of k-mer counts
##
## ->Syntax
...
##
## ->Required Parameters
##  - Data=<file[s]> : Starting dataset(s) (reads from these files are analyzed to generate your output).
##  -   This can include a wildcards (*) a directory (for which all fasta/fastq/fastq.gz/fastq.gz will be analyzed).
##  -   Rabbit will try to pair R1 and R2 files using the names, reporting aggregate results for each pair.  
##  -   As an alternative to a set of individual files, the user can specify a file that lists sample names and associated files
##  -      a line-by-line format using "SampleToData=<myfile>" where <myfile> is a simple text file where each line
##  -      starts with a sample name, followed by a tab and a comma-delimited list of relevant file paths
...
##
## ->Some of the many parameters that may be worth exploring sooner rather than later
##  - (all have defaults, so you can ignore all of these and just run the program) 
...
NoDup1 = True ## (nodup=) Setting NoDup to True instructs Rabbit to not to count k-mers in a paired end read that are also in R1.  This provides somewhat more nuanced K-mer counts but will slow the program down substantially; Read counts are not affected.
...
##  -
##  - Other Notes
##  - Requires Python 3.7+.  For speed, Python 3.11 (or better, PyPy3.9+) is highly recommended
##  - For SRR/ERR/DRR datasets not present LocalRepeatly, KFR will try to download the files from NCBI (linux/mac only).
##  -   You can provide the location of fastq-dump with FastQDumpProgram= (otherwise, KFR will look for it, which can be slow)
##  - Memory management
##  -     If Reference complexity is extensive (>500M different kmers), while Data is a smaller dataset, then
##  -     setting PrimaryIndex = 'Reference' can avoid memory issues (avoids preassembling a preliminary kmer Set from Datas)
##  -     PrimaryIndex is assigned by program by default but can be set manually to "Data" or "Reference" (whichever has fewer k-mers) if there are out-of-memory errors which in some cases can overcome insufficient memory situations
## End list of user settings

from VSG_ModuleFM import *
vcommand()
...
def CountWorker1(f1=0,sn='fromfeed',fL1='fromfeed',tools='',TurboS1=''):  #f1 is the sample number [0 based], sn is the sample name, fL1 is the file list, tools are inherited values for single thread calculation
    if tools: ## single core mode
        C1,C2,ReferenceFileList1,RefCount,ReadNumC,TetTrim,TSet,TurboRareK = tools
        myTask = f1,fL1,sn
    else:  ## multi core mode
        C1,C2,ReferenceFileList1,RefCount,ReadNumC,TetTrim,TSet,TurboRareK = vGrab("BasicData")
        myTask = vEat()
    maskS0 = 2**(2*TurboRareK)-1
    maskS1 = 2**(2*TurboRareK-3)-1
    shiftS1 = 2*TurboRareK-3
    if TurboRareK1 and Threads1:
        TurboS1 = array('B',[0]*(2**(2*TurboRareK-3)))
        for F1 in ReferenceFileList1:
            for s11 in vFastAToDict(F1,upper=True).values():
                ...
                for i1,L1 in enumerate(myIter):
                    if i1%DataCycle1==1:
                        ...
                        if i1>1 and i1%ReportLineGranularity1==1:
                            vLog('Counting Progress ' +myfn1+'Sample:'+sn+' RawReads:'+str(1+i1//DataCycle1)+' Matched:'+str(matchCount))
                        TotalReads += 1
                        if Paired:
                            L2 = L1[1].strip()
                            L1 = L1[0].strip()
                        ...
                        for j1,c1 in enumerate(L1):
                            ...
                            if j1>=StartMatching1:
                                vamin0 = min(a0,v0)
                                if TetTrim and (Tmask0&v0 in TSet): break
                                u = C1[vamin0]
                                if u==0: continue
                                myKmerC[vamin0] += 1
                                myKmerL[-u] += 1
                        ...
                        if myRef:
                            if 0>myRef>=-RefCount:
                                ...
                                myReadL[-myRef] += 1
                            matchCount += 1
                vLog('Counting Completed '+myfn1+'Sample:'+sn+' Reads:'+str(1+i1//DataCycle1)+' Matched:'+str(matchCount))            
            except:
...
if SampleToData1:
    ...
    for L1 in s2d1:
        for d1 in DataFileD1:
            if d1 in L1:
                DataFileD1[d1] = L1.split('\t')[0].strip()            
        for f1 in L1.replace(',',' ').replace('; ',';').replace('\t',' ').split():
            if ';' in f1:
                ...
                if (f1.lower().endswith('fasta.gz') or f1.lower().endswith('fa.gz') or f1.lower().endswith('fastq.gz') or \
                    ...
                    DataFileD1[(f1,f2)] = L1.split('\t')[0].strip()
                    ReadNumC1[f1] = 1
                    ReadNumC1[f2] = 2
            else:
                f1 = f1.strip(' "\t,'+"'")
                if (f1.lower().endswith('fasta.gz') or f1.lower().endswith('fa.gz') or f1.lower().endswith('fastq.gz') or \
                    ...
                    DataFileD1[f1] = L1.split('\t')[0].strip()
...
if PrimaryIndex1=='default':
    if TotalReferenceSize1<TotalDataSize1:
...
if PrimaryIndex1.lower() != 'reference' and PrimaryIndex1.lower() != 'data':
    ...
    if ReferenceFileByFile1:
        ...
        RefCount1 = 0
        for F1 in ReferenceFileList1:
            for L1 in vOpen(F1,mode='rt'):
                if L1.startswith('>'):
                    RefCount1 += 1

    for F1 in ReferenceFileList1:
        vLog('Starting Reference File PreScreen:',os.path.basename(F1))
        if ReferenceFileByFile1:
                    ...
                    if SkipDataPreIndex1 or (vamin1 in C1):
                        ...
                        if not(u1):
                            ...
                            if u1==-refnum1-2*RefCount1:
                                ...
                                C1[vamin1] = u1 + 2*RefCount1
                        elif u1<-RefCount1 and u1!=(-refnum1-RefCount1):
                            C1[vamin1] = u1 + RefCount1
                        elif not(PrivateOnly1):
...
if TurboRareK1 == 'Auto':
    ...
    if MemPerInstance1 > 0.8*MyMem1:
        TurboRareK1 -= 0
        MemPerInstance1 = 60*len(C1)+10**7 ## Extremely rough estimate of memory per instance
...
if AggregateUniqueAndLocalRepeats1:
    ...
    RefLocalRepeatKmerPositions1 =  RefPublicKmerPositions1
else:
...
if Threads1 == 0:
    for f1,fL1 in enumerate(MetaSampleList1):
        myKmerL1,myReadL1,TotalReads1,TotalKmers1,myKmerC1,myReadC1,PassFilterReads1,PassFilterKmers1 = CountWorker1(f1,fL1=fL1,sn=Samples1[f1],tools=[C1,C2,ReferenceFileList1,RefCount1,ReadNumC1,TetTrim1,TSet1,TurboRareK1],TurboS1=TurboS1)
        ...
        for (n11,f11),s11 in D11.items():
            ...
            if Express1:
                ...
                if FirstDibs1:
                    ...
                    JLocalRepeatKmerCounts1[(refnum1,f1)] += myKmerL1[refnum1+2*RefCount1]                    
                    JPublicKmerCounts1[(refnum1,f1)] += myKmerL1[refnum1]
                else:
                    JUniqueKmerCounts1[(refnum1,f1)] += myKmerL1[refnum1+2*RefCount1]
                    PublicSet1 = set()  # which Kmers have been encountered at this point
                    LocalRepeatSet1 = set()
                    v1 = 0; a1 = 0
                    StartMatching1 = klen1-1
                    for i1,c1 in enumerate(s11):
                        ...
                        if (i1>=StartMatching1):
                            ...
                            if 0>u1>=-RefCount1:
                                ...
                                if not(vamin1 in LocalRepeatSet1):
                                    if myc1:
                                        LocalRepeatSet1.add(vamin1)
                                        JLocalRepeatKmerCounts1[(refnum1,f1)] +=  myc1
                                        JLocalRepeatKmerCounts2[(refnum1,f1)] +=  myc1**2
                                if myc1:
                                    JLocalRepeatKmerCovered1[(refnum1,f1)] += 1
                            else:
                                if myc1:
                                    JUniqueKmerCovered1[(refnum1,f1)] += 1
                                    JUniqueKmerCounts2[(refnum1,f1)] += myc1**2
                RefMatches1[refnum1] += JUniqueKmerCounts1[(refnum1,f1)]+JPublicKmerCounts1[(refnum1,f1)]
                if not(AggregateUniqueAndLocalRepeats1):
                    RefMatches1[refnum1] += JLocalRepeatKmerCounts1[(refnum1,f1)]
                if not(PrivateOnly1):
                    RefMatches1[refnum1] += JPublicKmerCounts1[(refnum1,f1)]

 ## End of "duplicated block"
else:
    TaskList1 = []
    vShare([C1,C2,ReferenceFileList1,RefCount1,ReadNumC1,TetTrim1,TSet1,TurboRareK1],"BasicData")
...
## Start of "duplicated block"
                ...
                for (n11,f11),s11 in D11.items():
                    ...
                    if Express1:
                        JPublicReads1[(refnum1,f1)] += myReadL1[refnum1]
                        RefMatches1[refnum1] += JUniqueReads1[(refnum1,f1)]+JPublicReads1[(refnum1,f1)]
                        if not(AggregateUniqueAndLocalRepeats1):
                           RefMatches1[refnum1] += JLocalRepeatReads1[(refnum1,f1)]
                    else:
                        if FirstDibs1:
...
if not(type(ExtraTDVFields1) in (list,tuple)):
    ExtraTDVFields1 = [ExtraTDVFields1,]
...
ExtraTDVFields0 = [FieldNameD0[x] for x in ExtraTDVFields1]
ExtraTDVFields1 = [FieldNameD1[x] for x in ExtraTDVFields1]
...
for f1,fL1 in enumerate(MetaSampleList2):
    ...
    if not(SkipBlankData1) or SampleMatches1[f1]:
        if Express1:
            Columns1.extend(['ReadCount__'+sn1,])
            ColumnExtend1 = len(ExtraTDVFields1)
            Columns1.extend([x+'__'+sn1 for x in ExtraTDVFields1])
            sL1[0] += (1+ColumnExtend1)*( ','.join([os.path.basename(x) for x in fL1])+'\t')
            sL1[1] += (1+ColumnExtend1)*( str(f1+1)+'\t')
            sL1[2] += str(SampleTotalReads1[f1])+(1+ColumnExtend1)*'\t'
            sL1[3] += str(SamplePassFilterReads1[f1])+(1+ColumnExtend1)*'\t'
            sL1[4] += str(SampleUniqueReadMatches1[f1])+(1+ColumnExtend1)*'\t'
            if not(AggregateUniqueAndLocalRepeats1):
                sL1[5] += str(SampleLocalRepeatReadMatches1[f1])+'\t'
            if not(PrivateOnly1):
                sL1[6] += str(SamplePublicReadMatches1[f1])+(1+ColumnExtend1)*'\t'
        else:
            Columns1.extend(['KCount__'+sn1,'KCovered__'+sn1,'ReadCount__'+sn1])
            ColumnExtend1 = len(ExtraTDVFields1)
            Columns1.extend([x+'__'+sn1 for x in ExtraTDVFields1])
            TotalCovered1 = SampleUniqueKmerCovered1[f1]+SampleLocalRepeatKmerCovered1[f1]+SamplePublicKmerCovered1[f1]
            sL1[0] += (','.join([os.path.basename(x) for x in fL1])+'\t')*(3+ColumnExtend1)
            sL1[1] += (str(f1+1)+'\t')*(3+ColumnExtend1)
            sL1[2] += str(SampleTotalKmers1[f1])+'\t'+str(TotalCovered1)+'\t'+str(SampleTotalReads1[f1])+(1+ColumnExtend1)*'\t'
            sL1[3] += str(SamplePassFilterKmers1[f1])+'\t'+str(TotalCovered1)+'\t'+str(SamplePassFilterReads1[f1])+(1+ColumnExtend1)*'\t'
            sL1[4] += str(SampleUniqueKmerMatches1[f1])+'\t'+str(SampleUniqueKmerCovered1[f1])+'\t'+str(SampleUniqueReadMatches1[f1])+(1+ColumnExtend1)*'\t'
            if not(AggregateUniqueAndLocalRepeats1):
                sL1[5] += str(SampleLocalRepeatKmerMatches1[f1])+'\t'+str(SampleLocalRepeatKmerCovered1[f1])+'\t'+str(SampleLocalRepeatReadMatches1[f1])+(1+ColumnExtend1)*'\t'
            if not(PrivateOnly1):
                sL1[6] += str(SamplePublicKmerMatches1[f1])+'\t'+str(SamplePublicKmerCovered1[f1])+'\t'+str(SamplePublicReadMatches1[f1])+(1+ColumnExtend1)*'\t'
...
if Express1:
    MyTDVFields0 = [9,]+ExtraTDVFields0
else:
    MyTDVFields0 = [7,9,10]+ExtraTDVFields0
if AggregateUniqueAndLocalRepeats1:
...
def mydigit1(v):
    if v==0:
        ...
        return int(log(v,10))+1
    else:
        return int(log(v,10))+2
    ...
    for i1,myRef1 in [2,RefPublicReadsFound1[r1]],[1,RefLocalRepeatReadsFound1[r1]],[0,RefUniqueReadsFound1[r1]]:
        ...
        for f1 in treeFList1:
            ...
            for p1,j1 in enumerate(HeatMapTextFields0):
                fontsize3 = str(vclearance1)
                vtext(text=dis1(dD1[i1][j1][(r1,f1)]),xc=x11+xdel1/2,y1=y22-vclearance1,color=myTextColor1,font='dejavusans'+fontsize3)
                y22 -= vclearance1
...
x11 = 0
vclearance1 = int(round(vscale1))
vrect(x1=x11+4*xdel1,x2=x11+8*xdel1,y1=y11-4*vscale2,y2=y11-8*vscale2,fill='lightyellow',stroke='black',strokewidth=2.0)
for p1,j1 in enumerate(HeatMapTextFields0):
    fontsize3 = str(2*vclearance1)
    vtext(text=HeatMapTextFields1[p1],xc=x11+6*xdel1,y1=y22-3*vclearance1,color=black,font='dejavusansbold '+fontsize3)
    y22 -= 4*vclearance1
    vclearance1 = int(round(vclearance1/1.33))
...
for f1 in treeFList1: 
    ...
    vrect(x1=x11,x2=x11+xdel1,y1=y11,y2=y11+vscale2,fill='white',stroke='black',strokewidth=0.5)
    vtext(text=str(v1),xc=x11+xdel1/2,yc=y11+vscale2/2,color=black,font='dejavusans '+fontsize1,stroke='black')
    vtext(text=Samples1[f1],x1=x11+xdel1+3,yc=y11+vscale2/2,color=black,font='dejavusansbold '+fontsize2)
    y11 -= vscale2    
vlegend(text=' ')
myMaxKey1 = 1.0001*10**mydigit1(MaxCount1)/10
myMinKey1 = 0.9999*10**mydigit1(MinCount1)/10
vcolorkey(logmode=True, SpectrumTitle=HeatMapColorField1, mincolorindex=myMinKey1, maxcolorindex=myMaxKey1, colorvalue=myDisplayColor1, ckfont='dejavusansbold '+fontsize2)
