#!/usr/bin/env python
## CountRabbit: A fast Reference-based read/kmer counter for NGS Datasets
##
## Version ba08 03_12_2026
##
## OVERVIEW OF CountRabbit
## - You have a bunch of datasets from next generation sequencing and a reference, and want to count matches between them
## - CountRabbit is a tool providing tables of read match counts between sequence datan(fasta or fastq read files) and reference genome(s)
## - CountRabbit has some shared capabilities with standard tools like BLAST/Bowtie/Kallisto/BWA/Kraken and to many home-grown tools
## - The goals with CountRabbit are to provide a potentially simpler worflow for analysis
...
## -   CountRabbit is not designed to provided strandedness data, detect distant homology, or assemble sequencing data into contigs.
## -   CountRabbit is not designed for (or helpful in) detecting mutations, rearrangements, or termini
## -   CountRabbit provides a provisional (and customizable) graphic output, but this graphic is intended for
## -     quick experimental examination, while publication may require additional tools.
## - Specific features of CountRabbit overlap with local tools REVA,HQAlign,Polybench, Jazz18Heap, and many public tools
## - If everything is working properly, CountRabbit can be much faster to implement and/or run than any of these
## - CountRabbit is very much a work in progress- not recommended for production until we've tested it more thoroughly
...
##  -   you will need the module "VSG_ModuleFP.py" in the same folder (VSG_ModuleFN.py is on GITHUB Site)
...
##  -
## ->Some additional parameters to explore as you evaluate CountRabbit as a tool
...
## -> Some of CountRabbit is still being debugged-- so caveat mappor
## End Help
...
SampleToData1 = '' ## An alternative to a simple input data file or file list.  A file here specifies correspondence between samples and filenames.  Format: Each line should have SampleName <tab> comma-delimmited list of filenames
NoNs1 = False ## Setting this to true ignores any read with "N" bases 
AutoPair1 = True ## (autopair=) Setting this to true attempts to pair read1 and read2 files
R1Trim5 = 0 ## Length to trim from 5' end in R1
R2Trim5 = 0 ## Length to trim from 5' end in R2
R1Trim3 = 0 ## Length to trim from 3' end in R1
R2Trim3 = 0 ## Length to trim from 3' end in R2
MaxReadLen1 = 0 ## After filtering, reads will be truncated to this length. ==0 for no maximum read length
EndLinker1 = '' ## A linker at the end of the read will be chewed off using an rfind command (for short RNA reads one such is 'TGGAAT')
...
klen1 = 31 ## (k=) (kmer=) Kmer length for counting matched subsequences 
ReportGranularity1 = 1000000 ## How frequently to report updates
NoDup1 = False ## Setting NoDup to True instructs Rabbit to not to count k-mers in a paired end read that are also in R1.  This provides somewhat more nuanced K-mer counts but will slow the program down substantially; Read counts are not affected.

## Settings for has-based pre-screen (these don't affect the final output but can speed up the program in some cases)
BialyK1 = 'Auto' ## A KMer length to be used for initial prescreening.  This is particularly useful for datasets where most reads fail to match the reference
BialyLimit1 = 0 ## Setting this to a positive number n insists that there be a matching number within the first n bases of any read for it to be considered 
EstimatedAverageReadLen1 = 300 ## This only affects the speed-optimization (by guiding the choice of BialyK when it is set to "Auto"). Can change this if longer or shorter reads are expected.


## Settings to avoid counting of k-mers that match a preset file of sequencing adaptors
DetritisF1 = '' ## Name of detritis file for optional on-the-fly masking of linker bits (file illuminadetritis1223wMultiN.fa, distributed with this program, can be used)
...
DetritisK1 = 16 ## Length of K-mer for detritis filtering

## Settings for detailed handling of multimapping/repeated sequences
FirstDibs1 = False ## Setting this to true assigns each k-mer and read that is shared to only the first matching reference (disallows multimapping)
PrivateOnly1 = False ## Setting this to true ignores any k-mer present in more than one reference
AggregateUniqueAndLocalRepeats1 = False ## Setting this to true provides a single output that sums both unique and local repeats
AggregateUniqueAndPublic1 = False ## Setting this to true aggregates public k-mers with unique for each reference sequence.  Generally used with FirstDibs and AggregateUniqueAndLocalRepeats
## Settings to filter the index of reference K-mers used to generate counts

## Dust Filtering (inspired by work from R Agarwala and D Lipman
## k is any k-mer (e.g., 32-mer), SubKLen is a list of lengths to do dust filtering at (e.g., 3,6,9), DustThresholds is a set of thresholds (coincidence frequencies) above which a value of True  will be returned
...
DustSubKLen1=(3,6,9) ## These are the kmer lengths that we'll test complexity at (too few different subk-mers of any of these lengths will trigger a triaging of the parent kmer) 
DustThresholds1=(0.18,0.05,0.02) ## These are the maximum coincidences/max-coincidence thresholds for the different kmer lengths to be tested

## Filtering based on coverage of references from the present dataset
...
##   Otherwise the standard BialyK filtering should be fine.
##   And note that Kmer thinning and BialyK filtering don't synergize (the two together would rarely be better than either alone)
...
SkipBlankRefs1 = True ## Setting this to True will mask output for reference sequences that are not detected
SkipBlankData1 = True ## Setting this to True will mask output for datasets where no reference match is detected
Delimiter1 = '\r' ## New line delimiter for output
ExtendedHeader1 = True ## Setting this to True gives an extended descriptive header at the top of the tab delimited output file
Transpose1 = False ## Setting this to True provides a transposed output tdv file (references as columns, samples as rows)
...
VerySimple1 = False ## Setting this to true runs RabbitCount in a paired down mode that just looks for unique kmers and reports read counts 
SingleCount1 = False ## Setting this to true runs RabbitCount in a minimal mode where all reads "belonging" to a reference are grouped in one category 
...
## Some settings for graphic and .tsv output
vscale1 = 14 ## (vscale=) Vertical scale for graphic
hscale1 = 42 ## (hscale=) Horizontal scale for graphic
MinCountsForDisplay1 = 1 ## References with fewer average reads-per-sample will be skipped in display
...
ClusterReferences1 = False ## Setting clusterferences to true implements a very rudimentary references clustering for the graphic display 
...
ClusterSamples1 = False ## Setting clustersamples to true implements a very rudimentary samples clustering for the graphic display 
GraphTitle1 = 'default' ## Sets a graph title.  Default will be OutputFile Name
...
DisplayGraphImmediately1 = True ## (immediate=) Setting this to false will suppress immediate graphic display of SVG file
## Choice of output fields
#     'KmerCount2'unt'               :  7   ## Raw Kmer Counts 
...
TSVFields1 = 'default'      ## (tsvfield=) (tsvfields=) Specifying one or more of the "processed" measurement values above (#11-19 above) in this list expands the .tsv output to include these 
...
##  - Turning on the option PrivateOnly=True tells CountRabbit to ignore public kmers and reads (focusing only on k-mers belonging to a single reference)
##  -
...
##  -      Express=True tells CountRabbit to assign each read based on the first uniquely matching k-mer encountered (no need to go through the rest of the read)
...
try:
    from VSG_ModuleFW import *
except:
    try:
        from VSG_ModuleFP import *
    except:
        print()
        print('********** ERROR: Failed to Import either module VSG_ModuleFP.py or VSG_ModuleFV.py                          ***********')
...
if VerySimple1:
    PrivateOnly1 = True
    FirstDibs1 = True
    AggregateUniqueAndLocalRepeats1 = True
    Express1 = True
if SingleCount1:
    FirstDibs1 = True
    AggregateUniqueAndLocalRepeats1 = True
    AggregateUniqueAndPublic1 = True
    Express1 = True
    if not('extendedheader' in ''.join(sys.argv).lower()):
        ExtendedHeader1 = False
    if TSVFields1=='default':
        TSVFields1 = [9]
    
    
myvnow1 = vnow.replace('_','').replace('T','_').replace('D','_')
if not('pypy' in sys.version.lower()):
    vLog('\n*** ADVISORY *** CountRabbit running in Standard Python: will be faster if run in PyPy (see PyPy.org)\n')
def myOpen1(fn,mode):
...
mask1 = 4**(klen1-1)-1
ksam1 = 4**(klen1-1)
Tmask1 = 4**(DetritisK1-1)-1
Tmask0 = 4**DetritisK1-1
Tsam1 = 4**(DetritisK1-1)
DetritisK1m1 = DetritisK1-1
...
def SimpleDust1(k):
    ...
    ## sequences of intermediate complexity will have an intermediate c/mc
    ## Heuristic exploration leads to a provisional recommendation of subkmer lengths of SubKLen=(3,6,9) for 32-mer words, with thresholds >0.18, >0.05, >0.02 for calling a given k-mer degenerate
    for z,(dustk,filtk) in enumerate(zip(DustSubKLen1,DustThresholds1)):
        dustmask = 4**dustk-1
        c = Counter()
...
def CountWorker1(f1=0,sn='fromfeed',fL1='fromfeed',tools='',BialyS1=''):  #f1 is the sample number [0 based], sn is the sample name, fL1 is the file list, tools are inherited values for single thread calculation
    if tools: ## single core mode
        C1,C2,ReferenceFileList1,RefCount,ReadNumC,TetTrim,TSet,BialyK,Threads = tools
        myTask = f1,fL1,sn
    else:  ## multi core mode
        C1,C2,ReferenceFileList1,RefCount,ReadNumC,TetTrim,TSet,BialyK,Threads = vGrab("BasicData")
        myTask = vEat()
    maskS0 = 2**(2*BialyK)-1
    maskS1 = 2**(2*BialyK-3)-1
    shiftS1 = 2*BialyK-3
    if BialyK and Threads:
        BialyS1 = array('B',[0]*(2**(2*BialyK-3)))
        for F1 in ReferenceFileList1:
            for n11,s11 in vFastAToDict(F1,upper=True).items():
                ...
                a11 = vantisense(s11)
                for p1 in range(len(s11)-BialyK+1):
                    myHash = hash(s11[p1:p1+BialyK])&maskS0
                    myIndex = myHash & maskS1
                    myBit = myHash >> shiftS1
                    BialyS1[myIndex] |= 1<<myBit
                    myHash = hash(a11[p1:p1+BialyK])&maskS0
                    myIndex = myHash & maskS1
                    myBit = myHash >> shiftS1
                    BialyS1[myIndex] |= 1<<myBit
    while myTask:
        ...
        for F1  in fL1:
            ...
            try:
                ...
                for i1,L1 in enumerate(myIter):
                    if i1%DataCycle1==1:
                        ...
                        if BialyK:
                            KeepMe1 = False
                            p1 = klen1-BialyK
                            while not(KeepMe1) and p1+BialyK<=len1:
                                myHash = hash(L1[p1:p1+BialyK])&maskS0
                                myIndex = myHash & maskS1
                                myBit = myHash >> shiftS1
                                if BialyS1[myIndex]&(1<<myBit):
                                    KeepMe1 = True
                                else:
                                    p1 += klen1-BialyK+1
                                    if BialyLimit1 and p1+BialyK>BialyLimit1: break
                            p1 = klen1-BialyK
                            if Paired:
                                while not(KeepMe1) and p1+BialyK<=len2:
                                    myHash = hash(L2[p1:p1+BialyK])&maskS0
                                    myIndex = myHash & maskS1
                                    myBit = myHash >> shiftS1
                                    if BialyS1[myIndex]&(1<<myBit):
                                        KeepMe1 = True
                        ...
                        for j1,c1 in enumerate(L1):
                            ...
                            if j1>=StartMatching1:
                                if DataCadence1 and not(j1%DataCadence1): continue
                                vamin0 = min(a0,v0)
                                u = C1[vamin0]
                                if u==0: continue
                ...
                vLog('Failure of Counting for Data File:'+os.path.basename(F1)+' Sample:'+sn+' Reads:'+str(i1//DataCycle1)+' Matched :'+str(matchCount))
        if tools:
...
print('******************')
print('******************')
print('Alpha Version of CountRabbit-- this version for testing only (no guarantees of anything working or being correct/useful')
print('If more current versions of CountRabbit are available they will be at: github.com/FireLabSoftware/CountRabbit ')
print('******************')
print('******************')
print()
...
if SampleToData1:
    ...
    for L1 in s2d1:
        ...
        for f1 in L1.replace(',',' ').replace('; ',';').replace('\t',' ').strip().split():
            if ';' in f1:
                f1,f2 = f1.split(';',1)
                f1 = f1.strip(' "\t,'+"'").strip()
                f2 = f2.strip(' "\t,'+"'").strip()
                ...
                for ff1 in sorted(list(glob(f1))):
                    if (ff1.lower().endswith('fasta.gz') or ff1.lower().endswith('fa.gz') or ff1.lower().endswith('fastq.gz') or \
                        ff1.lower().endswith('fasta') or ff1.lower().endswith('fa') or ff1.lower().endswith('fastq')) and os.path.isfile(ff1):
                        DataFileD1[ff1] = mySample1
        for f1 in L1.split('\t'):
            f1 = f1.strip()
...
if TotalReferenceSize1>2*10**8:
    vLog('\n\n*** ADVISORY *** \n    Your reference file is large ('+str(TotalReferenceSize1/1000000)+'mb)\n'+
    '    if you get a memory error, try rerunning with \n'+
    "    PrimaryIndex='data' \n     (note this only works if your dataset is small or limited complexity, but worth a try)\n\n")
...
if OutFileName1 == 'default':
    Chaser1 = '.tsv'
    pm1 = 'CountRabbitMatchCounts_'
    OutFileBase1 = FileListMnemonic1(DataFileList2)
    if OutFileBase1=='x':
        OutFileName1 = pm1+myvnow1+Chaser1            
    else:
        OutFileName1 = pm1+OutFileBase1+'_'+myvnow1+Chaser1
    OutFilePath1 = os.getcwd()
else:
    OutFileBase1 = os.path.basename(OutFileName1)
    OutFilePath1 = os.path.dirname(OutFileName1)
    if OutFileBase1.endswith('.tsv') or OutFileBase1.endswith('.svg'):
        OutFileBase1 = OutFileBase1[:-4]
...
if TSVFields1=='default':
    if Express1:
        TSVFields1 = 'ReadCount'
    else:
        TSVFields1 = 'ReadCount','KmerCount','KmersCovered'
if not(type(TSVFields1) in (list,tuple)):
    TSVFields1 = [TSVFields1,]
...
TSVFields0 = [FieldNameD0[x] for x in TSVFields1]
TSVFields1 = [FieldNameD1[x] for x in TSVFields1]
HeatMapTextFields0 = [FieldNameD0[x] for x in HeatMapTextFields1]
HeatMapTextFields1 = [FieldNameD1[x] for x in HeatMapTextFields1]
AllFields1 = [HeatMapColorField0,]+TSVFields0+HeatMapTextFields0
...
if DetritisF1:
    ...
    TetTrim1 = False
for FnT1 in multiglob1(DetritisF1):
        ...
        for i1,L1 in enumerate(F1o):
            if i1%DataCycle1==1:
                ...
                for j1,c1 in enumerate(L1):
                    ...
                    a0 = (a0>>2)+(3-cd1)*ksam1
                    vamin0 = min(v0,a0)
                    if TetTrim1 and j1>=DetritisK1m1 and (Tmask0&v0 in TSet1):
                        StartMatching1 = j1+klen1
                    if j1>=StartMatching1:
                        C1[vamin0] = 0
        myClose1(F1o)
...
if PrimaryIndex1.lower() != 'reference' and PrimaryIndex1.lower() != 'data':
        ...
        for F1 in ReferenceFileList1:
            ...
            for n11,s11 in D1.items():
                ...
                for i1,c1 in enumerate(s11):
                    ...
                    v1 = ((v1&mask1)<<2)+cd1
                    a1 = (a1>>2)+(3-cd1)*ksam1
                    if TetTrim1 and i1>=DetritisK1m1 and (Tmask0&v1 in TSet1):
                        StartMatching1 = i1+klen1
    ...
    if FilterList1:
        ...
        for g1 in FilterList1:
            ...
            if gf1.lower()=='reference':
                ...
                myseqD1 = map(vFastAToDict,multiglob1(gf1))
            for Dx1 in myseqD1:
            ...
            C1 = C1temp2
    if DustFilter1:
        DustSubKLen1 = [int(x) for x in DustSubKLen1]
        DustThresholds1 =[float(x) for x in DustThresholds1]
    ...
    if FrequencyFilter1:
        ...
        for x1 in xD0:
            ...
            if not(HotKMerF1): HotKMerF1=1 
            xI2 = int(ceil(lx1*HotKMerF1)-1)
            if lx1:
                xD2[x1] = xD0[x1][xI2]
            else:
                xD2[x1] = 1
...
if WriteIndex1:
    if WriteIndex1 == True or WriteIndex1.lower()=='reference':
        WriteIndex1 = 'CountRabbitIndex_'+myvnow1+'.pck'
...
if BialyK1 == 'Auto':
    if DataCadence1 or ReferenceCadence1:
        ...
        if BialyLimit1:
            SampleReadLen1 = min(BialyLimit1,EstimatedAverageReadLen1)
        BialyK1 = ceil(log(1+len(C1)*20*SampleReadLen1/(klen1-15),2)/2) ## ~~ 95% of non-matching reads should be triaged
        if BialyK1 < 14:
            BialyK1 = 14
        try:
        ...
        MemPerInstance1 = (4**BialyK1)//8+60*len(C1)+10**7 ## Extremely rough estimate of memory per instance
        if MemPerInstance1 > MaxMemPerThread1*MyMem1:
            BialyK1 -= 1
            MemPerInstance1 = (4**BialyK1)//8+60*len(C1)+10**7 
        if MemPerInstance1 > MaxMemPerThread1*MyMem1:
            BialyK1 -= 1
            MemPerInstance1 = (4**BialyK1)//8+60*len(C1)+10**7 
        if MemPerInstance1 > MaxMemPerThread1*MyMem1:
        ...
        if MemPerInstance1>MyMem1:
            ...
            Threads1 = 0
            BialyK1 -= 0
        vLog('BialyK setting: '+str(BialyK1))
...
if Threads1 == 'Auto' or Threads1==True:
    ...
    Threads1 = 0
if BialyK1 and Threads1==0:
    BialyS1 = array('B',[0]*(2**(2*BialyK1-3)))
    maskS0 = 2**(2*BialyK1)-1
    maskS1 = 2**(2*BialyK1-3)-1
    shiftS1 = 2*BialyK1-3
    for s11 in D11.values():
        a11 = vantisense(s11)
        for p1 in range(len(s11)-BialyK1+1):
            myHash = hash(s11[p1:p1+BialyK1])&maskS0
            myIndex = myHash & maskS1
            myBit = myHash >> shiftS1
            BialyS1[myIndex] |= 1<<myBit
            myHash = hash(a11[p1:p1+BialyK1])&maskS0
            myIndex = myHash & maskS1
            myBit = myHash >> shiftS1
            BialyS1[myIndex] |= 1<<myBit
else:
...
if AggregateUniqueAndPublic1:
    RefPublicKmerPositions1 = RefUniqueKmerSpecies1
    RefPublicKmerSpecies1 =  RefUniqueKmerPositions1
else:
...
JUniqueCounts1 = Counter()
JUniqueCounts2 = Counter()
JUniqueKmerCovered1 = Counter()
if AggregateUniqueAndPublic1:
    JPublicReads1 = JUniqueReads1
    JPublicCounts1 = JUniqueCounts1
    JPublicCounts2 = JUniqueCounts2
    JPublicKmerCovered1 = JUniqueKmerCovered1
else:
    JPublicReads1 = Counter()
    JPublicCounts1 = Counter()
    JPublicCounts2 = Counter()
    JPublicKmerCovered1 = Counter()
if AggregateUniqueAndLocalRepeats1:
    JLocalRepeatReads1 = JUniqueReads1
    JLocalRepeatCounts1 = JUniqueCounts1
    JLocalRepeatCounts2 = JUniqueCounts2
    JLocalRepeatKmerCovered1 = JUniqueKmerCovered1
else:
    JLocalRepeatReads1 = Counter()
    JLocalRepeatCounts1 = Counter()
    JLocalRepeatCounts2 = Counter()
...
if Threads1 == 0:
    for f1,fL1 in enumerate(MetaSampleList1):
        myKmerL1,myReadL1,TotalReads1,TotalKmers1,myKmerC1,myReadC1,PassFilterReads1,PassFilterKmers1 = CountWorker1(f1,fL1=fL1,sn=Samples1[f1],tools=[C1,C2,ReferenceFileList1,RefCount1,ReadNumC1,TetTrim1,TSet1,BialyK1,Threads1],BialyS1=BialyS1)
## Start of "duplicated block"
        ...
        if not(Express1):
            ...
            FoundPublicK1 = sum([myReadL1[i] for i in range(1,RefCount1+1)])
        refnum1 = 0
        if KeepCounts2:
            myCounts2 = Counter()
            for k1,n1 in myKmerC1.items():
                myCounts2[C1[k1]] += n1**2
        for (n11,f11) in D11:
            refnum1 += 1
            ...
            if not(Express1):
                ...
                JUniqueCounts1[(refnum1,f1)] += myKmerL1[refnum1+2*RefCount1]
                JLocalRepeatCounts1[(refnum1,f1)] += myKmerL1[refnum1+RefCount1]
                if KeepCounts2:
                    JUniqueCounts2[(refnum1,f1)] += myCounts2[-refnum1-2*RefCount1]
                    JLocalRepeatCounts2[(refnum1,f1)] += myCounts2[-refnum1-RefCount1]
                if FoundPublicK1 and not(PrivateOnly1): 
                    if FirstDibs1:
                        JPublicCounts1[(refnum1,f1)] += myKmerL1[refnum1]
                        JPublicKmerCovered1[(refnum1,f1)] += CoverageByRef1[refnum1]
                        if KeepCounts2:
                            JPublicCounts2[(refnum1,f1)] += myCounts2[-refnum1]
                    ...
                    if (i1>=StartMatching1):
                        ...
                        if 0>u1>=-RefCount1:
                            ...
                            if not(Express1):
                                ...
                                if not(vamin1 in PublicSet1):
                                    PublicSet1.add(vamin1)
                                    JPublicCounts1[(refnum1,f1)] +=  myc1
                                    JPublicKmerCovered1[(refnum1,f1)] += 1
                                    if KeepCounts2:
                                        JPublicCounts2[(refnum1,f1)] +=  myc1**2
            ...
            if not(PrivateOnly1) and not(AggregateUniqueAndPublic1):
                RefMatches1[refnum1] += JPublicReads1[(refnum1,f1)]

 ## End of "duplicated block"
else:
    TaskList1 = []
    vShare([C1,C2,ReferenceFileList1,RefCount1,ReadNumC1,TetTrim1,TSet1,BialyK1,Threads1],"BasicData")
...
RefPublicKmersFound1 = [sum([JPublicCounts1[(r1,f1)] for f1 in range(SampleNum1)]) for r1 in range(RefCount1+1)]
RefLocalRepeatKmersFound1 = [sum([JLocalRepeatCounts1[(r1,f1)] for f1 in range(SampleNum1)]) for r1 in range(RefCount1+1)]
RefUniqueKmersFound1 = [sum([JUniqueCounts1[(r1,f1)] for f1 in range(SampleNum1)]) for r1 in range(RefCount1+1)]
...
SampleUniqueKmerMatches1 = SamplesSum1(JUniqueCounts1)
SampleLocalRepeatKmerMatches1 = SamplesSum1(JLocalRepeatCounts1)
SamplePublicKmerMatches1 = SamplesSum1(JPublicCounts1)
...
OutFile1 = myOpen1(os.path.join(OutFilePath1,OutFileName1), mode='wt')
...
TaskHeader1 += '<!--RunTime: '+myvnow1+'-->'+Delimiter1
...
Columns1 = ['RefName',
                 ...
                 'RefLen',
                 'RefKCount']
if AggregateUniqueAndLocalRepeats1 and AggregateUniqueAndPublic1:
    PrivateUnique1 = 'All'
elif AggregateUniqueAndLocalRepeats1:
    PrivateUnique1 = 'Private'
else:
    PrivateUnique1 = 'Unique'
AllRefK1 = AllRefUniqueKmer1
if not(PrivateOnly1) and not(AggregateUniqueAndPublic1):
    AllRefK1 += AllRefLocalRepeatKmer1
if not(AggregateUniqueAndLocalRepeats1):
    AllRefK1 += AllRefPublicKmer1
sL1 = ['Sample_Info\tFiles\t \t \t \t \t',
                    'Sample_Info\tSample_IDNum\t \t \t \t \t',
                    'Sample_Info\tTotal\t \t \t \t \t',
                    'Sample_Info\tPassFilter\t \t \t \t \t',
                    'Pan_Reference\tUnique\tAll\t \t \t'+str(AllRefUniqueKmer1)+'\t',
                    'Pan_Reference\tLocalRepeat\tAll\t \t \t'+str(AllRefLocalRepeatKmer1)+'\t',
                    'Pan_Reference\tPublic\tAll\t \t \t'+str(AllRefPublicKmer1)+'\t',
                    'Pan_Reference\tAll\tAll\t \t'+str(AllRefLen1)+'\t'+str(AllRefK1)+'\t']
# 0 'Sample_Info\tFiles\t\t\t\t\t',
# 1 'Sample_Info\tSample_IDNum\t \t \t \t \t',
# 2 'Sample_Info\tTotal\t \t \t \t \t',
# 3 'Sample_Info\tPassFilter\t \t \t \t \t',
# 4 'All_Reference\tUnique\tAll\t \t \t'+str(AllRefUniqueKmer1)+'\t',
# 5 'All_Reference\tLocalRepeat\tAll\t \t \t'+str(AllRefLocalRepeatKmer1)+'\t',
# 6 'All_Reference\tPublic\tAll\t \t \t'+str(AllRefPublicKmer1)+'\t',
# 7 'All_Reference\tAll\tAll\t \t \t'+str(AllRefK1)+'\t']
...
for f1,fL1 in enumerate(MetaSampleList2):
    ...
    if not(SkipBlankData1) or SampleMatches1[f1]:
        Columns1.extend([x1+'__'+sn1 for x1 in TSVFields1])
        for x1 in TSVFields1:
            if type(fL1)==str:
                sL1[0] +=  os.path.basename(fL1)+'\t'
            else:
            ...
            if x1 == 'ReadCount':
                ...
                AllMatches1 = SampleUniqueReadMatches1[f1]
                if not(PrivateOnly1) and not(AggregateUniqueAndPublic1):
                    AllMatches1 += SamplePublicReadMatches1[f1]
                if not(AggregateUniqueAndLocalRepeats1):
                    AllMatches1 += SampleLocalRepeatReadMatches1[f1]
                sL1[7] += str(AllMatches1)+'\t'
            elif x1 == 'KmerCount':
                ...
                AllMatches1 = SampleUniqueKmerMatches1[f1]
                if not(PrivateOnly1) and not(AggregateUniqueAndPublic1):
                    AllMatches1 += SamplePublicKmerMatches1[f1]
                if not(AggregateUniqueAndLocalRepeats1):
                    AllMatches1 += SampleLocalRepeatKmerMatches1[f1]
                sL1[7] += str(AllMatches1)+'\t'
                ...
                AllMatches1 = SampleUniqueKmerCovered1[f1]
                if not(PrivateOnly1) and not(AggregateUniqueAndPublic1):
                    AllMatches1 += SamplePublicKmerCovered1[f1]
                if not(AggregateUniqueAndLocalRepeats1):
                    AllMatches1 += SampleLocalRepeatKmerCovered1[f1]
                sL1[7] += str(AllMatches1)+'\t'
            else:
                for myi1 in range(2,8): sL1[myi1] += ' \t'           
...
for sI1,sL0 in enumerate(sL1):
    if sI1==5 and AggregateUniqueAndLocalRepeats1: continue
    if sI1==6 and AggregateUniqueAndPublic1 or PrivateOnly1: continue
    if sI1==7 and AggregateUniqueAndLocalRepeats1 and AggregateUniqueAndPublic1: continue
    OutFile1.write(sL0+' \t'+Delimiter1)
MyTSVFields0 = TSVFields0
...
for r1 in range(1,RefCount1+1):
    if not(SkipBlankRefs1) or RefMatches1[r1]:
        ...
        if not(AggregateUniqueAndLocalRepeats1) and RefLocalRepeatKmerSpecies1[r1]:
            ...
            OutFile1.write('\t'.join(map(str,OutItems1))+'\t'+Delimiter1)
        if not(PrivateOnly1) and not(AggregateUniqueAndPublic1) and RefPublicKmerSpecies1[r1]:
...
myClose1(OutFile1)
if Transpose1:
    fF = open(os.path.join(OutFilePath1,OutFileBase1+'.tsv'),mode='rt').readlines()
    fTrF = open(os.path.join(OutFilePath1,'TR_'+OutFileBase1+'.tsv'), mode='wt')
    myLine = 0
    myHeaderLine = ''
    myC = Counter()
    for L1 in fF:
        if L1.startswith('<!--') or not(L1.strip()): continue
        if not(myHeaderLine):
            myHeaders = L1.strip().split('\t')
        for myColumn,myEntry in enumerate(L1.strip().split('\t')):
            myC[(myColumn,myLine)] = myEntry    
        myLine+=1
    if ExtendedHeader1:
        fTrF.write(TaskHeader1)
        fTrF.write(HeaderTranspose1(myHeaders))
    for myColNum,myColHead in enumerate(myHeaders):
        for i in range(myLine):
            val = myC[(myColNum,i)]
            if not(val): val = ' '
            fTrF.write(val+'\t')
        if myColNum==0:
            fTrF.write(AbbrevHeader1+'\t '+Delimiter1)
        fTrF.write('\t '+Delimiter1)    
        if(myColNum==len(myHeaders)-1): break    
    fTrF.close()
...
RefCommonL1 =[]
for r1 in range(1,RefCount1+1):
    if RefUniqueReadsFound1[r1]+RefLocalRepeatReadsFound1[r1]*(not(AggregateUniqueAndLocalRepeats1))+RefPublicReadsFound1[r1]*(not(PrivateOnly1 or AggregateUniqueAndPublic1))>=MinCountsForDisplay1:
        RefCommonL1.append(r1)
if SortReferencesByReadCount1:
    RefCommonL1 = sorted(RefCommonL1, key=lambda x:RefUniqueReadsFound1[x]+RefLocalRepeatReadsFound1[x]*(not(AggregateUniqueAndLocalRepeats1))+RefPublicReadsFound1[x]*(not(PrivateOnly1 or AggregateUniqueAndPublic1)))
nRefCommon1 = len(RefCommonL1)
JAllReads1 = Counter()
JAllReads1 += JUniqueReads1
if not(AggregateUniqueAndLocalRepeats1):
    JAllReads1 += JLocalRepeatReads1
if not(PrivateOnly1 or AggregateUniqueAndPublic1):
...
if ClusterSamples1:
    ...
    for f1 in range(SampleNum1):
        ...
        for f2 in range(SampleNum1):
            ...
            distC1[(f1,f2)] = euclid1(myvec1,myvec2) #euclid1(myvec1,myvec2)  #kljointdistance1(myvec1,myvec2)
...
if ClusterReferences1:
    ...
    for r1 in RefCommonL1:
        ...
        for r2 in RefCommonL1:
            ...
            distC2[(r1,r2)] = euclid1(myvec1,myvec2) #euclid1(myvec1,myvec2)  #kljointdistance1(myvec1,myvec2)
...
if PrivateOnly1 or AggregateUniqueAndPublic1:
...
for r1 in treeRList1[::-1]:
    for i1,myRef1 in [2,RefPublicReadsFound1[r1]],[1,RefLocalRepeatReadsFound1[r1]],[0,RefUniqueReadsFound1[r1]]:
        if MinCountsForDisplay1 and not(myRef1): continue
        if i1==2 and not(RefPublicKmerSpecies1[r1]): continue
        if i1==1 and not(RefLocalRepeatKmerSpecies1[r1]): continue
        if i1==0 and not(RefUniqueKmerSpecies1[r1]): continue
...
if SampleCategory1:
    ...
    CategoryBoxWidth1 = float(fontsize1)*0.7*maxCategoryLen1
for f1 in treeFList1: 
    ...
    if SampleCategory1:
        ...
        vtext(text=SampleCategory1[Samples1[f1]],xc=x11+xdel1+3+CategoryBoxWidth1/2,yc=y11+vscale2/2,color=SampleLabelColor1[Samples1[f1]],font='dejavusansbold '+fontsize1)
        StartLabel1 += CategoryBoxWidth1+3
    vtext(text=Samples1[f1],x1=StartLabel1,yc=y11+vscale2/2,color=SampleLabelColor1[Samples1[f1]],font='dejavusansbold '+fontsize2)
    y11 -= vscale2    
vlegend(text=' ')
if GraphTitle1 == 'default':
    if OutFileBase1=='x':
        GraphTitle1 = 'CountRabbit_Summary_'+myvnow1
    else:
        GraphTitle1 = 'CountRabbit_Summary_'+OutFileBase1+'_'+myvnow1
vtitle(text=' ', font="dejasansbold "+str(vscale2))
vtitle(text=GraphTitle1, font="dejasansbold "+str(int(0.8*vscale2)))
if MinCount1:
    ...
    vcolorkey(logmode=True, SpectrumTitle=HeatMapColorField1, keyheight=vscale2, mincolorindex=myMinKey1, maxcolorindex=myMaxKey1, colorvalue=myDisplayColor1, ckfont='dejavusansbold '+fontsizeCK1)
if not(SuppressGraphDetails1):
    vlegend(text=' ')
    vlegend(text=vSysLogInfo1, font='dejavusans 4')
if OutFileBase1=='x':
    vdisplay(os.path.join(OutFilePath1,'CountRabbitSVG_'+myvnow1+'.svg'),display=DisplayGraphImmediately1)
else:
    vdisplay(os.path.join(OutFilePath1,'CountRabbitSVG_'+OutFileBase1+'_'+myvnow1+'.svg'),display=DisplayGraphImmediately1)
...
## 03_24_24 aa00  First version
## 07_13_24 ak12  First version in alpha stage
## 03_10_26 ba07  Fixed a minor issue in the parsing of filenames
## 03_12_26 ba08  Fixed a problem in graphic display of low count experiments